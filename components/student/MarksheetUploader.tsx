...
export default function MarksheetUploader() {
  ...
  async function loadDocuments() {
    ...
    setDocuments(
      docs.filter((doc: any) => doc.document_type === "marksheet")
    );
  }
  ...
  async function handleUpload(
    ...
    try {
      ...
      await uploadFile("marksheets", path, file);
  ...
  async function handleDelete(doc: Document) {
    await deleteFile("marksheets", doc.file_path);
  ...
  async function handlePreview(path: string) {
    const url = await getFileUrl("marksheets", path);
      ...
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-slate-400">
            Upload up to 3 academic marksheets (PDF or image).
          </p>
        </div>
