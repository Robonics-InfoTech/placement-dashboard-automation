...
export default function CertificateUploader() {
  ...
  async function loadDocuments() {
    ...
    setDocuments(
      docs.filter((doc: any) => doc.document_type === "certificate")
    );
  }
  ...
  async function handleUpload(
    ...
    if (documents.length >= 5) {
      alert("You can upload a maximum of 5 certificates.");
      return;
    }
    ...
    try {
      ...
      await uploadFile("certifications", path, file);
  ...
  async function handleDelete(doc: Document) {
    await deleteFile("certifications", doc.file_path);
  ...
  async function handlePreview(path: string) {
    const url = await getFileUrl("certifications", path);
      ...
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-slate-400">
            Upload up to 5 certifications (PDF or image).
          </p>
        </div>
